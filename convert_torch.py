...
def torch_to_pytorch(t7_filename,outputname=None):
    model = load_lua(t7_filename,unknown_classes=True,  long_size=8)  #WINDOWS
    #model = load_lua(t7_filename, unknown_classes=True) # LINUX
...
#m = m:float()
#export = args.model + "cpu.t7"
#torch.save(export,m)

#m = load_lua('converted.t7', long_size=8)

torch_to_pytorch(args.model,args.output)
#torch_to_pytorch(m, args.output)